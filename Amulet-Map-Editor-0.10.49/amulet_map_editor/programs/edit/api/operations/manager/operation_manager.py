...
class BaseOperationManager:
    ...
    def _iter_entry_points(self, group: str) -> Iterable:
        try:
    ...
    def _load_entry_points(self):
        group = f"amulet_map_editor.edit_operations.{self._group_name}"
        for ep in self._iter_entry_points(group):
            try:
                obj = ep.load()
            except BaseException:
                log.warning(
                    f"Failed loading entry point {ep.name} from {ep.value}.\n{traceback.format_exc()}"
                )
                continue

            if isinstance(obj, ModuleType):
                self._load_module(f"{group}:{ep.name}", obj)
            elif isinstance(obj, dict):
                self._load_operation(f"{group}:{ep.name}", obj)
            elif isinstance(obj, (list, tuple)):
                for i, export_dict in enumerate(obj):
                    if isinstance(export_dict, dict):
                        self._load_operation(f"{group}:{ep.name}[{i}]", export_dict)
                    else:
                        log.error(
                            f"Invalid export entry point {ep.name} index {i}. Expected dict."
                        )
            else:
                log.error(
                    f"Entry point {ep.name} must load a module or export dict/list."
                )
