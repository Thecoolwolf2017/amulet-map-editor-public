...
from amulet_map_editor import lang, close_level, open_level
...
class EditExtension(wx.Panel, BaseProgram):
    ...
    def __init__(self, parent, world: "World"):
        ...
        self._skip_unsaved_check = False
    ...
    def menu(self, menu: MenuData) -> MenuData:
        ...
        menu.setdefault(lang.get("menu_bar.file.menu_name"), {}).setdefault(
            ...
            lambda evt: self._restore_latest_backup_workflow(),
        )
        ...
        menu.setdefault(lang.get("menu_bar.options.menu_name"), {}).setdefault(
            ...
            "Backup Settings",
            lambda evt: self._edit_backup_settings(),
        )
    ...
    def _edit_backup_settings(self):
        ...
        dialog = SimpleDialog(self, "Backup Settings")
        sizer = wx.BoxSizer(wx.VERTICAL)
        dialog.sizer.Add(sizer, 0, wx.ALL | wx.EXPAND, 5)

        enabled_checkbox = wx.CheckBox(
            dialog, label="Enable automatic backup before save/convert"
        )
        enabled_checkbox.SetValue(backups_enabled())
        sizer.Add(enabled_checkbox, 0, wx.ALL | wx.EXPAND, 5)

        root_label = wx.StaticText(dialog, label="Backup folder")
        sizer.Add(root_label, 0, wx.LEFT | wx.RIGHT | wx.TOP, 5)
        backup_root_picker = wx.DirPickerCtrl(
            dialog,
            path=backup_root_dir(),
            message="Select the backup folder",
            style=wx.DIRP_USE_TEXTCTRL,
        )
        sizer.Add(backup_root_picker, 0, wx.ALL | wx.EXPAND, 5)

        retention_sizer = wx.BoxSizer(wx.HORIZONTAL)
        retention_sizer.Add(
            wx.StaticText(dialog, label="Retained backups per world"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            8,
        )
        retention_spin = wx.SpinCtrl(
            dialog, min=1, max=1_000, initial=backup_retention_count()
        )
        retention_sizer.Add(retention_spin, 0, wx.ALIGN_CENTER_VERTICAL)
        sizer.Add(retention_sizer, 0, wx.LEFT | wx.RIGHT | wx.BOTTOM, 5)

        latest_backup = get_latest_backup(self._world.level_path)
        latest_path = latest_backup["backup_path"] if latest_backup else "None found"
        latest_backup_text = wx.StaticText(
            dialog,
            label=f"Latest backup for this world:\n{latest_path}",
        )
        latest_backup_text.Wrap(700)
        sizer.Add(latest_backup_text, 0, wx.ALL | wx.EXPAND, 5)

        restore_button = wx.Button(dialog, label="Restore Latest Backup")
        restore_button.Enable(latest_backup is not None)
        sizer.Add(restore_button, 0, wx.ALL | wx.EXPAND, 5)

        def _restore_latest(_):
            dialog.EndModal(wx.ID_CANCEL)
            self._restore_latest_backup_workflow()

        restore_button.Bind(wx.EVT_BUTTON, _restore_latest)
        dialog.Fit()

        if dialog.ShowModal() == wx.ID_OK:
            set_backup_settings(
                enabled_checkbox.GetValue(),
                backup_root_picker.GetPath(),
                retention_spin.GetValue(),
            )

    def _restore_latest_backup_workflow(self):
        latest_backup = get_latest_backup(self._world.level_path)
        if latest_backup is None:
            wx.MessageBox("No backup is available for this world yet.")
            return

        backup_path = latest_backup["backup_path"]
        confirm = wx.MessageDialog(
            self,
            f"This will restore the world from:\n{backup_path}\n\nContinue?",
            "Restore Latest Backup",
        ...
        world_path = self._world.level_path
        self._skip_unsaved_check = True
        close_level(world_path)
        self._skip_unsaved_check = False
        wx.CallLater(150, self._restore_and_reopen_world, world_path, backup_path)

    @staticmethod
    def _restore_and_reopen_world(world_path: str, backup_path: str):
        try:
            restore_backup(backup_path, world_path)
            open_level(world_path)
        except Exception as exc:
            wx.MessageBox(f"Failed to restore backup:\n{exc}")
            return
        wx.MessageBox("Backup restore complete.")
