...
class LangTestCase(unittest.TestCase):
    def test_lang(self):
        lang_root = Path(__file__).resolve().parents[1] / "amulet_map_editor" / "lang"
        self.assertTrue(
            lang_root.is_dir(), f"Language directory not found: {lang_root}"
        )
        for lang_dir in (str(lang_root),):
